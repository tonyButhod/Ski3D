...
{
    read_obj("../meshes/fence.obj", m_positions, m_indices, m_normals, m_texCoords);
    ...
    sf::Image image;
    image.loadFromFile("../textures/wood.jpg");
...
void Fence::do_keyPressedEvent( sf::Event& e )
{
}

void Fence::setMaterial(const MaterialPtr& material)
{
    m_material = material;
}
...
void addFences(HierarchicalRenderablePtr system, ShaderProgramPtr texShader, int nb_fences, glm::mat4 transform) {
    ...
    for (int i=0; i<nb_fences; ++i) {
        FencePtr fenceLeft = std::make_shared<Fence>(texShader);
        fenceLeft->setMaterial(normal);
        fenceLeft->setParentTransform(transform*translate);
        HierarchicalRenderable::addChild(system, fenceLeft);
        translate = glm::translate(translate, glm::vec3(0.0,0.0,5.0));
    }
}